...
resolvedverips <- data.frame(ip = NA, ASN = NA)
resolvedsolips <- data.frame(ip = NA, ASN = NA)
tmpdf1 <- data.frame()
tmpdf2 <- data.frame()
...
myverips <- as.character(serverversion$Requester)
...
################################################
#Geo-Reference the Solution IPs
################################################

#get just the Requesting IPs from the data frame
mysolips <- as.character(solution$Requester)
#get a list of unique IPs to make the lookup less expensive
uniquesolips <- na.omit(unique(unlist(mysolips)))
uniquesolips <- list(uniquesolips)

for (j in 1:length(uniquesolips[[1]])){
  resolvedsol <- geoaslookup(uniquesolips[[1]][j])
  tmpdf2 <- data.frame(ip = uniquesolips[[1]][j],ASN = resolvedsol )
  resolvedsolips <- rbind(resolvedsolips,tmpdf2)
}

resolvedsolips <- na.omit(resolvedsolips)

#now time to merge the resolved ASN info with the list of info
finalsoldata <- merge(solution,resolvedsolips, by.x = "Requester", by.y = "ip", all=T)
#Reorder output columns
finalsoldata <- finalsoldata[c(2,3,1,4)]
#Get rid of list formatting to enable writing to file
